 ...
 cardEntryMode=? AND businessDate=? AND restaurantID=?");
	...
	function buildSBSLine($data){
		$return="
			  ...
			  <td style=\"text-align:center;\">".$data['fooda']['Count']."</td>
			  <td style=\"text-align:center;\">$".$this->switchNegNumber($data['fooda']['Total'],0)."</td>
			  <td style=\"text-align:right;\">".trim($data['lyfooda']['Count'])."</td>
			  <td style=\"border-right:1px solid #e3e6ea;text-align:center;\">$".$this->switchNegNumber($data['lyfooda']['Total'],0)."</td>
...
pbc2.pbc_ToastCheckHeaders.ToastOrderID=pbc2.pbc_ToastOrderHeaders.GUID";
    ...
    final public function getTippedOrders(): array {
		...
		AND pbc_ToastOrderPayment.ToastCheckID NOT IN (SELECT orderGUID FROM pbc2.pbc_TipDistribution) AND pbc_ToastOrderPayment.GUID NOT IN (SELECT guid FROM pbc_DeliveryRequests) AND pbc_ToastOrderPayment.ToastCheckID NOT IN (SELECT GUID FROM pbc_ToastCheckHeaders WHERE tabName LIKE '%Grubhub%')";
		$stmt = $this->mysqli->prepare($q);
		$stmt->execute();
		$result = $stmt->get_result();
		while($row=$result->fetch_object()){$r[]=$row;}
