...
final class Toast {
    ...
    function loadGUIDs() {
        ...
        while ($row = $result->fetch_object()) {
            $this->restOptions[$row->GUID] = $row->optionName;
        }
    }
...
//		if(trim($do)=="" || !isset($do)) {return "BLANK";echo "NO do\n";}
        if (array_key_exists($do, $this->restOptions)) {
            return $this->restOptions[$do];
        }
...
//		if($result->status!=200 && $result->status!="") {$this->notifyIT(json_decode($result)."\n\n Authorization: Bearer " . $this->auth->accessToken."Toast-Restaurant-External-ID: " . $this->guid."\n\n".$this->url. "/orders/v2/orders?businessDate=" . $date,"JSON Error - getOrders");}
    ...
    function getOrdersByTime($date, $s, $e, $tz) {
        ...
        $result = curl_exec($ch);
        if (isset($result->status) && $result->status != 200 && $result->status != "") {
    ...
    function storeSelectionInfo($s, $ToastCheckID, $stmt): void {
        ...
        if (isset($s->salesCategory->guid) && $s->salesCategory->guid != "") {
            $salesCategories = $this->getRestaurantOptions($s->salesCategory->guid, 'salesCategories');
        } else {
            $salesCategories = "None";
        }
    ...
    function storeTaxInfo($t, $CheckItemID, $stmt) {
        if (!isset($t->guid) || $t->guid == '') {
            $t->guid = $this->genGUID(microtime());
        }
    ...
    function storePaymentInfo($p, $ToastCheckID, $restaurant, $date, $stmt) {
        ...
        if (!is_numeric($p->originalProcessingFee)) {
            $p->originalProcessingFee = 0.00;
        }
        if (!is_numeric($p->last4Digits)) {
            $p->last4Digits = 0;
        }
        if (isset($p->refund->refundAmount) && $p->refund->refundAmount != 0.00) {
            $refund = json_encode($p->refund);
        } else {
            $refund = "None";
        }
    ...
    function storeOrderInfo($json, $stmt) {
        ...
        if (isset($json->diningOption->guid) && $json->diningOption->guid != "") {
            $diningOptions = $this->getRestaurantOptions($json->diningOption->guid, 'diningOptions');
        } else {
        ...
        if (isset($json->wageOverrides[0])) {
            foreach ($json->wageOverrides as $w) {
                if ($this->getLatestWage($json->guid) != $w->wage) {
                    ...
                    $stmt1->execute();
                    if ($stmt1->error != '') {
                        $this->notifyIT("ToastEmployeeWages \n\n" . $stmt1->error . "\n\n" . $this->restaurantID . "\n" . $json->guid . "\n" . $jc->title . "\n" . $w->wage, "SQL Import Error");
                    }
                }
            }
        }
    ...
    function storeCashInDB($json, $stmt): void {
        ...
        if (isset($json->payoutReason->guid) && $json->payoutReason->guid != '') {
            $payoutReasons = $this->getRestaurantOptions($json->payoutReason->guid, 'payoutReasons');
        } else {
            $payoutReasons = "";
        }
        if (isset($json->noSaleReason->guid) && $json->noSaleReason->guid != '') {
            $noSaleReason = $this->getRestaurantOptions($json->noSaleReason->guid, 'noSaleReasons');
        } else {
            $noSaleReason = "";
        }
