...
<div id='queryResults'>
    ...
    if(file_exists($report->docSaveLocation.$fileName.date("Ymd").'.csv')) {
			$ret.="<div>
      <a class='btn btn-success' href='".$report->docDownloadLocation.$fileName.date("Ymd").".csv' target='_blank')\">Download the file</a> This download is only valid for 30 minutes.
      </div>";
		}
    "
$ret.=  </div>";
